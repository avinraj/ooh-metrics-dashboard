...
import { byTelecomOperator } from "../../../../Data/mobileAdReports.json";
import MobileAdChartTable from "./MobileAdChartTable";
import { Box, Typography, useMediaQuery, useTheme } from "@mui/material";
import { mobileAdChartDataAndOptions } from "./utils/chartUtils";

const TelecomOperatorChart = () => {
  const { t } = useTranslation();
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));

  const tableColumns = Object.keys(byTelecomOperator[0]).map((key) => ({
  ...
  const processData = () => {
    const groupedData = byTelecomOperator.reduce(
      (
        acc: Record<
          string,
        ...
        const key = `${item.telecomOperator}`;
        if (acc[key]) {
    ...
    <div>
      ...
      >
        <Typography variant="h3">
          {" "}
          {t("reports.telecomOperator.byTelecomOperator")}
        </Typography>
      </Box>
      <MobileAdChartTable
        tableData={byTelecomOperator}
...
export default TelecomOperatorChart;
