...
const ReportsAcc: React.FC<ReportsAccProps> = ({ onOpenSwitchModal, icon }) => {
  ...
  useEffect(() => {
    if (selectedMenu) {
      if (selectedMenu === "impressions" || selectedMenu === "demographics" || selectedMenu === "affinities") {
    ...
    <div>
      ...
      >
        <AccordionSummary
          ...
          expandIcon={<ExpandMoreIcon sx={{ color: selectedVal ? "black" : theme.palette.text.primary }} />}
        >
          <Box display={"flex"} alignItems={"center"}>
            {icon}
            <Typography sx={{ color: selectedVal ? "black" : theme.palette.text.primary }}>
              {t("sideBar.reports")}
            </Typography>
          </Box>
        </AccordionSummary>
          ...
          {reportsConfig.map(({ title, value }, index) => (
            <Box
              ...
              sx={{
                ...
                borderBottomLeftRadius: reportsConfig.length === index + 1 ? "5px" : 0,
                borderBottomRightRadius: reportsConfig.length === index + 1 ? "5px" : 0,
                backgroundColor: selectedVal === value ? theme.palette.primary.main : theme.palette.secondary.main,
                cursor: "pointer",
                // "&:hover": {
            ...
            >
              <Typography
                variant="body1"
                sx={{
                  color: selectedVal === value ? "black" : theme.palette.text.primary,
                  fontSize: "15px",
                  paddingBlock: "10px",
                  paddingInline: "12px",
                }}
              ...
              {reportsConfig.length === index + 1 ? null : <Divider component="li" />}
            </Box>
          ))}
        </AccordionDetails>
      </Accordion>
